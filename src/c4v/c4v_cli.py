...
def c4v_cli():
    ...
    if not os.path.isdir(DEFAULT_FILES_FOLDER):
        click.echo(f"[INFO] Creating local files folder at: {DEFAULT_FILES_FOLDER}", err=True)
        try:
...
@click.option('--loud',     is_flag=True,   help="Print scraped data to stdio as it is being scraped")
@click.option('--up-to',    default=-1,     help="Maximum amount of elements to scrape. Scrape all if no argument given")
@click.argument('urls', nargs = -1)
def scrape(urls : List[str] = None, files : bool = None, loud : bool = False, up_to : int = -1):
    """
...
def crawl(crawlers : List[str] = [], list : bool = False, all : bool = False, all_but : bool = False, loud : bool = False):
    """
    ...
    for crawler in crawlers_to_run:
            ...
            def process(list : List[str]):
                scraped_data = [ScrapedData(url=url) for url in db_manager.filter_scraped_urls(list)]
                db_manager.save(scraped_data)
...
@click.option('--scraped-only', default = None, help='Retrieve only complete rows, those with its scraped data')
def list(urls : bool = False, limit : int = 100, col_len : int = 50, scraped_only : bool = None):
    """
    ...
    scraped_only = (scraped_only == 'true' or scraped_only == 'True' or scraped_only == '1')
    ...
    data_to_print = data_list_to_table_str([d for d in db_manager.get_all(limit, scraped_only)], max_cell_len=col_len)
