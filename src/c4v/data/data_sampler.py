...
class DataSampler:
    ...
    def cleaner(self, df, is_pandas_series=False):
        ...
        if not is_pandas_series:
            df[self.text_col] = DataCleaner.data_prep_4_annotate(df[self.text_col])
        else:
            df = DataCleaner.data_prep_4_annotate(df)

        df = DataCleaner.remove_newlines(df[self.text_col])
